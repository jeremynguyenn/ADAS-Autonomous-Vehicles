...
void NRF24_TxMode (uint8_t *Address, uint8_t channel)
{
	...
	config = config | (1<<1);    // write 1 in the PWR_UP bit for TX
...
void NRF24_RxMode (uint8_t *Address, uint8_t channel)
{
	...
	nrf24_WriteReg(RX_ADDR_P1, 0xEE);  // Write the Pipe2 LSB address
...
//check the data is available in fifo status register
uint8_t isDataAvailable (int pipenum)
{
	...
	{
		//clear the rxdr bit in status register
...
//copy data from pipe to our buffer
